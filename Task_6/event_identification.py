...
# Theme: 			Geo Guide
# Functions:			detect_markers, arena_coordinates, get_corner_coordinates, arena_extraction, frame_capture, event_extraction, coord_overlap, pixelate_image, bilateral_denoising, increase_saturation, event_label_prediction, bounding_box,event_mapping, map_priority_to_events, event_bot_mapping, return_4A_helper
...
Logic: This function creates the lists required for the arena_coordinates function and then calculates the coordinates of the corners of the arena
	...
	passed as arguements to the arena_coordinates function which finds the corrdinates of the arena 
	corners.
...
Logic: This function extracts the images from the arena images and their coordinates
	...
	The arena image is grayscaled (for thresholding) and the contours are detected using a threshold which is incrememted.
	...
	colour in local areas (such as a Gaussian Blur), the homogenity of the image remains the same.
	
	The event image is then denoised by applying a sharpening kernel and added to the "present_images" array along with the 
	coordinates in the "present_coordinates" array.
...
'''
Function Name: pixelate_image
Input: image - input image
	pixel_size - size of the pixel in the input image
Output:	pixelated input image
Logic: This function pixelates the input image

	This function is used in the event_extraction function to compute the SSD
Example Call: img = pixelate(image, pixel_size)
'''
...
Logic: This function applies the bilateral filter on the input image
...
Logic: This function saturates the input image
...
Input: present_edge_list - list containing the mapping between the priority edges and the detected events which is sorted
	based on the priority of visiting
	priority_edges - dictionary containing the events defining edge
...
Logic: This function maps the events to their numbers in order of their priority.
	...
	The priorties are reflected in the order of the present_edge_list list (output of the previous function).
	If the edges in the priorites are present in the priority_edges then it is assigned the event number and 
...
def printing_function(identified_labels):
	...
	for key in identified_labels.keys():
		if identified_label[key] ==  "combat":
			new_dict[key] = "Combat"
		elif identified_label[key] ==  "human_aid_rehabilitation":
			new_dict[key] = "Humanitarian Aid and rehabilitation"
		elif identified_label[key] ==  "fire":
			new_dict[key] = "Fire"
		elif identified_label[key] ==  "military_vehicles":
			new_dict[key] = "Military Vehicles"
		elif identified_label[key] ==  "destroyed_buildings":
