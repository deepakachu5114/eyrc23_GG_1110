...
*        		===============================================
*
*  This script is to track the live location of the bot for task 6 of Geo Guide (GG) Theme (eYRC 2023-24).
*
...
#all_corners: the path to a csv file lat_long.csv containing arena aruco id's along with the respective latitude, longitude of the streets
all_corners = "/home/deepakachu/Desktop/eyantra_stage_2/experimetation/lat_long.csv"
...
def read_csv(csv_name):
    ...
    with open(csv_name, 'r') as csvfile:
        ...
        # Add the Latitude and Longitude from each row of the CSV file to the lat_lon dictionary. Use the ArUco ID as
        # the index/key for each entry in the dictionary
...
'''
    ...
    Output: None
    Logic: Tracks the nearest ArUco marker to the bot and writes its coordinates to a CSV file
    Example Call: tracker(100, lat_lon)
'''
def tracker(ar_id, lat_lon):
    # Convert ar_id to a string
    ar_id = str(ar_id)
    # Check if the given ArUco marker ID (ar_id) is present in the lat_lon dictionary. If it is, fetch the corresponding
    # coordinate from the dictionary using the ArUco marker ID as the index. Subsequently, write this coordinate to a CSV file utilizing the write_csv function.
    if ar_id in lat_lon:
...
'''
    ...
    Output: None
    Logic: Takes in the frame sent by the communication.py file and analyses it to find the nearest ArUco to the bot's 
            aruco marker.
            
            We maintain a list to keep track of the last 5 aruco markers the bot passed by (the last 5 tracked locations)
            to avoid back-tracking (the QGIS dot should not move backwards).
            
            We then iterate through the detected list of aruco markers and find the distance between each of those and the
            bot's aruco marker. If the distance is less than a certain, threshold value, we consider that aruco as the
            representation of the location at which the bot is present at that moment and update a CSV file that will
            be tracked using the QGIS software.
...
def tracking(frame, ret):
    ...
    if ids is not None and 100 in ids:
        ...
        for index, value in enumerate(ids):
            # We omit the corner arucos because these hold no relevance in tracking. To avoid false alarms, its best to
            # remove these.
            if value in [4, 5, 6, 7]:
        ...
        dist = 99999
        # loc :  stores the ArUco marker ID (location) that is currently considered closest to the reference marker of
        # the bot (ID 100). We initialise it to an arbitrary value, will be updated later.
        loc = 900
        # threshold : maximum acceptable distance between an aruco marker and the bot's aruco marker to consider the bot
        # is nearest to that aruco (consequently, the QGIS dot will represent this aruco on the map).
        threshold = 31.5

        # Get corners of ArUco marker with ID 100 (the bot)
        corners1 = corners[ids.index(100)]

        # Loop through detected ArUco markers except the bot's ID
        for id in ids:
            if id != 100:
                corners2 = corners[ids.index(id)]
                # Calculate center coordinates of the bot's aruco marker
                # The four corners of detected ArUco marker are represented as liste of (x,y) coordinates. Each corner
                # corresponds to one of the four corners of the marker
        ...
        if dist <= threshold:
            ...
            if loc not in last_5_unique_points:
                tracker(loc, lat_lon)
                # Add the current ArUco marker ID 'loc' to the list last_5_unique_points only if it is not already
                # present in the list so that it won't be mis-tracked.
