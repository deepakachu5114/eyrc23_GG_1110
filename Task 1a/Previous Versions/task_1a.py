...
def data_preprocessing(task_1a_dataframe):
	...
	new_columns = ['PaymentTier','Age','Yes','No','Bachelors','Masters',	2012,2013,2014,2015,2016,2017,2018,'ExperienceInCurrentDomain','Bangalore','New Delhi','Pune','Male','LeaveOrNot']
...
def load_as_tensors(features_and_targets):
	...
	tensors_and_iterable_training_data = [X_train_tensor,X_test_tensor,y_train_tensor,y_test_tensor,[X_train_tensor,y_train_tensor]]
	##########################################################
...
class Salary_Predictor(torch.nn.Module):
	...
	def __init__(self):
		...
		'''
		#######	ADD YOUR CODE HERE	#######
		self.linear1 = torch.nn.Linear(18, 72)        
		self.activation1 = torch.nn.ReLU()
		self.linear2 = torch.nn.Linear(72, 36)                   
		self.activation2 =torch.nn.ReLU()
		self.linear3 = torch.nn.Linear(36, 1)            
...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
	...
	for e in range(number_of_epochs):
		for inputs,label in zip(tensors_and_iterable_training_data[0],tensors_and_iterable_training_data[2]):
			output = model(inputs)
...
def validation_function(trained_model, tensors_and_iterable_training_data):
	...
	with torch.no_grad():
		for inputs, labels in zip(tensors_and_iterable_training_data[1],tensors_and_iterable_training_data[3]):
			outputs = trained_model(inputs)

			predicted_labels = (outputs > 0.5).float()

			if predicted_labels == labels[0]:
