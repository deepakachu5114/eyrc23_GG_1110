...
min_contour_perimeter = 120  # Adjust this based on the border thickness
max_contour_perimeter = 250  # Adjust this based on the size of the enclosed images
...
for i, contour in enumerate(contours):
    ...
    if min_contour_perimeter < perimeter < max_contour_perimeter:
        ...
        extracted_resized = cv2.resize(sharpened_extracted_image, (50, 50))
        images.append(extracted_resized)
        coordinates.append([x,y,w,h])


for i in range(1, len(images)+1):
    cv2.imwrite(f'extracted_image_{i}.png', images[i-1])


model = tf.keras.models.load_model(r"C:\Users\adith\Jupyter Projects\Deep Learning\Task4\model.pth")
